...
function validatePixKey(key: string, type: PixKeyType): string | null {
  ...
  switch (type) {
    case "cpf":
      if (!/^\d{11}$/.test(clean)) return "CPF deve conter 11 dígitos numéricos.";
      break;
    case "cnpj":
      if (!/^\d{14}$/.test(clean)) return "CNPJ deve conter 14 dígitos numéricos.";
      break;
    case "email":
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(trimmed)) return "E-mail inválido.";
      if (trimmed.length > 255) return "E-mail muito longo.";
      break;
    case "telefone":
      if (!/^\+?\d{10,13}$/.test(clean)) return "Telefone deve ter entre 10 e 13 dígitos (com ou sem +).";
      break;
    case "aleatoria":
      if (trimmed.length < 10 || trimmed.length > 100) return "Chave aleatória deve ter entre 10 e 100 caracteres.";
      break;
  }
  return null;
}

export default function PixManual() {
  ...
  useEffect(() => {
    document.title = "PIX Manual | Gestor IPTV";
  }, []);
  ...
  return (
    <div>
      ...
      <main className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-4 items-start">
        {/* Status - aparece primeiro em ambos */}
        ...
        {/* Chave PIX - segunda posição mobile, coluna direita desktop */}
        <Card className="shadow-sm min-h-[320px] lg:row-span-2">
          <CardHeader>
        ...
        {/* Salvar - terceiro mobile, abaixo do status no desktop */}
        <Card className="shadow-sm">
